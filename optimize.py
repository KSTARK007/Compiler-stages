...
Temporaries that are never assigned to any variable nor used in any expression are deleted. Done recursively.
	...
	print('to_remove',temps_to_remove)
	new_list_of_lines = []
	for line in list_of_lines :
		tokens = line.split()
...
def constant_propagation(list_of_lines):
	...
	for line in list_of_lines:
		tokens = line.split()
		if(len(tokens) == 3):
			if (istemp(tokens[0]) and tokens[2].isdigit()):
...
if __name__ == "__main__" :
	...
	printicg(without_deadcode, "dead code removal")
	...
	printicg(folded_constants, "Post constant folding")

	print("STARTING PROPAGATION")
	propagated = constant_propagation(folded_constants)
	printicg(propagated,"Post constant_propagation")
