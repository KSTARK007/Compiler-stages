...
%token	CHAR INT FLOAT VOID USING SYSTEM
...
%type <nPtr> statement compound_statement expression_statement block_item expression_statement_no
...
expression_statement_no
    : ';'
    | expression {$$ = $1;}

iteration_statement
	: FOR '(' expression_statement expression_statement expression ')' compound_statement {$$ = opr(FOR, 4, $3, $4, $5, $7);}
	|
	  IF '(' expression_statement_no ')' compound_statement {$$ = opr(IF, 2, $3, $5);}
	|
	  WHILE '(' expression_statement_no ')' compound_statement {$$ = opr(WHILE,2,$3,$5);}
	;
...
external_declaration
	: USING SYSTEM ';' IDENTIFIER IDENTIFIER compound_statement {ex($6, 0); /*freeNode($2);*/}
	| declaration							{
												if(if_assign)
												{											
													ex($1, 2); /*freeNode($2);*/
